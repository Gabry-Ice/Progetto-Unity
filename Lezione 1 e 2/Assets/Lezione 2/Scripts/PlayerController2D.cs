...
public class PlayerController2D : MonoBehaviour {
    ...
    private void ApplyGravity() {
        ...
        if (lockZ) direction.z = 0.0f; // non permette il movimento sull'asse Z
    }

    private void ApplyRotation() {
        if (input.sqrMagnitude == 0) return;

        direction = Quaternion.Euler(0.0f, lockZ ? 0.0f : mainCamera.transform.eulerAngles.y, 0.0f) * new Vector3(input.x, 0.0f, lockZ ? 0.0f : input.y);

        var flatDirection = new Vector3(direction.x, 0.0f, direction.z);
        if (flatDirection.sqrMagnitude == 0) return;
    ...
    public void Move(InputAction.CallbackContext context) {
        ...
        direction = new Vector3(input.x, 0.0f, lockZ ? 0.0f : input.y);
    }
